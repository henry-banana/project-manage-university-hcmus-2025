...
SQLiteConnector::SQLiteConnector(const std::string& dbPath)
    ...
    // std::cout << "SQLiteConnector created for path: " << _dbPath << std::endl; // Debug
}

SQLiteConnector::~SQLiteConnector() {
    // std::cout << "SQLiteConnector for path " << _dbPath << " being destroyed." << std::endl; // Debug
    disconnect();
}
...
SQLiteConnector::SQLiteConnector(SQLiteConnector&& other) noexcept
    ...
    // std::cout << "SQLiteConnector move constructed for path: " << _dbPath << std::endl; // Debug
}
...
SQLiteConnector& SQLiteConnector::operator=(SQLiteConnector&& other) noexcept {
    if (this != &other) {
        ...
        // std::cout << "SQLiteConnector move assigned for path: " << _dbPath << std::endl; // Debug
    }
    return *this;
}
...
bool SQLiteConnector::connect() {
    if (_db) { // Đã kết nối rồi
        // std::cout << "SQLiteConnector::connect - Already connected to " << _dbPath << std::endl; // Debug
        return true;
    }
    ...
    if (rc != SQLITE_OK) {
        std::cerr << "SQLiteConnector Error: Cannot open database '" << _dbPath << "': " << sqlite3_errmsg(_db) << std::endl;
        sqlite3_close(_db); // Quan trọng: close db handle ngay cả khi open thất bại để giải phóng tài nguyên
        _db = nullptr;
        return false;
    }
    // std::cout << "SQLiteConnector::connect - Successfully connected to " << _dbPath << std::endl; // Debug
    return true;
}
...
bool SQLiteConnector::executeNonQuery(const std::string& sqlQuery) {
    if (!_db) {
        std::cerr << "SQLiteConnector Error: Database not connected. Cannot execute query." << std::endl;
        return false;
    }
    char* errMsg = nullptr;
    int rc = sqlite3_exec(_db, sqlQuery.c_str(), nullptr, nullptr, &errMsg);
    if (rc != SQLITE_OK) {
        std::cerr << "SQLiteConnector SQL error in executeNonQuery: " << errMsg << " (Query: " << sqlQuery << ")" << std::endl;
