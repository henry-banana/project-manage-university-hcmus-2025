...
public:
    ...
    // addOrUpdate vì điểm có thể được nhập lần đầu hoặc cập nhật
    ...
    virtual OperationResult<bool> removeAllForStudent(const std::string& studentId) = 0; // (➕) Khi xóa SV
    virtual OperationResult<bool> removeAllForCourse(const std::string& courseId) = 0; // (➕) Khi xóa Course
};
